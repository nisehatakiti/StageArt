...
## 20. Production Context：「小屋入り～本番」メニュー

### 20.1 位置づけ

Productionの管理業務は、稽古までの準備期間だけで完結しない。

公演直前の小屋入りから各Performanceの本番運営までを、Production Context内の独立した業務領域として扱う。

そのため、Production Contextの左サイドメニューに「小屋入り～本番」を追加し、その配下に以下の機能を配置する。

    小屋入り～本番
    ├─ タイムテーブル
    └─ 受付

「小屋入り～本番」は、稽古と同列の単一機能ではなく、本番運営に関する機能群をまとめる親カテゴリとして扱う。

### 20.2 Production Contextの正式メニュー構造

Production ContextのContext Areaは、現時点の正本として以下の構造を採用する。
...
固定領域である「ホーム」「マイページ」「設定」「ログアウト」は、Production Contextに入っても置き換えない。

「公開情報」は、Application側でProduction Public Pageに表示する内容、公開状態、公開日時・公開タイミング等を管理する入口であり、Public Siteそのものではない。

### 20.3 タイムテーブル

「タイムテーブル」は、小屋入りから本番終了までの当日運営に必要な時間進行を作成・確認するProduction管理機能とする。

タイムテーブル画面では、少なくとも以下の操作へ到達できるようにする。

- タイムテーブルを作成
- タイムテーブルを印刷

画面上の主要アクションは明示的に以下の名称で提供する。

    ＋ タイムテーブルを作成
    🖨 タイムテーブルを印刷

「タイムテーブルを作成」は、StageArt上で小屋入り～本番の時間進行を作成・編集する操作である。

「タイムテーブルを印刷」は、作成済みタイムテーブルを当日の出演者・スタッフが利用できる印刷用形式で出力する操作である。

タイムテーブル画面は単なるSchedule一覧の別名ではなく、**本番当日の運営用時間進行を作成・利用する業務画面**として扱う。

### 20.4 受付

「受付」は、Productionの本番運営における受付業務の入口とする。

受付画面では、新たな受付方式を画面ごとに独自定義するのではなく、**Blueprint上ですでに確定している受付処理を一覧として表示し、当日必要な受付処理を選択して実行できる構造**とする。
...
各受付処理の具体的な内容・順序・QRコード受付等の詳細は、既存の受付処理仕様を正本とする。

QRコード読み取り等、端末固有の操作特性を必要とする受付処理については、既存の「QR受付等の専用画面」方針に従い、通常のサイドメニュー画面とは別の専用画面へ遷移してよい。

ただし、その場合も受付画面自体はProduction Context内の「小屋入り～本番」→「受付」から到達する入口として統一する。

### 20.5 時間軸上の業務整理

Production Contextのメニューは、概念上以下の業務フェーズを自然に表現する。
...
これによりStageArtを、公開情報や参加者の管理だけを行うシステムではなく、**公演の準備から小屋入り・本番当日の運営までを一貫して支援するApplication**として明確に位置づける。

### 20.6 確定事項

- Production Contextの左サイドメニューに「小屋入り～本番」を追加する。
- 「小屋入り～本番」の配下には「タイムテーブル」「受付」を配置する。
- 「小屋入り～本番」は本番運営に関する機能群をまとめる親カテゴリとして扱う。
- タイムテーブル画面では「タイムテーブルを作成」「タイムテーブルを印刷」の操作を提供する。
- 受付画面では、以前に確定した受付処理を一覧表示し、対象処理を選択して実行できる入口とする。
- 受付処理の具体仕様は既存の受付処理Blueprintを正本とし、本項で勝手に再定義しない。
- QRコード等の専用操作が必要な受付処理は専用画面へ遷移してよいが、Production Context内の「小屋入り～本番」→「受付」を共通入口とする。
