...
use StageArt\Domain\Authentication\PasswordResetTokenId;
use StageArt\Domain\Authentication\PasswordResetTokenRepositoryInterface;
use StageArt\Domain\UserAccount\UserAccountId;
use wpdb;

final class WordPressPasswordResetTokenRepository implements PasswordResetTokenRepositoryInterface
{
    private wpdb $wpdb;
    ...
    {
        $this->wpdb = $wpdb;
        $this->table = $wpdb->prefix . 'stageart_password_reset_tokens';
    }
    ...
    private function hydrate(array $row): PasswordResetToken
    {
        return PasswordResetToken::reconstitute(
            PasswordResetTokenId::fromString($row['id']),
