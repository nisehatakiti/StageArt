...
export default function RegistrationPendingScreen() {
  ...
  return (
    <AuthLayout>
      ...
      <ThemedText style={authStyles.description}>
        {email ? (
          <>
      ...
      <TouchableOpacity
        ...
        disabled={resending}
        style={[authStyles.button, resending && authStyles.buttonDisabled]}
      >
        {resending ? <ActivityIndicator color="#fff" /> : <ThemedText style={authStyles.buttonText}>確認メールを再送する</ThemedText>}
      </TouchableOpacity>

      {resendMessage && (
        <ThemedText testID="registration-pending-resend-feedback" type="small" style={authStyles.description}>
          {resendMessage}
        </ThemedText>
      )}
...
const styles = StyleSheet.create({
  emailText: { fontWeight: '700', color: STAGE.inputText },
});
