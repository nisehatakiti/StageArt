...
/**
 * Rehearsal.md's 6 Status values, only 3 of which this UI's own actions
 * can currently produce (create -> SCHEDULED, confirm -> CONFIRMED,
 * cancel -> CANCELLED); DRAFT/ACTIVE/COMPLETED are included for
 * completeness since GET can still return them (e.g. direct API use)
 * but no mobile-rn control reaches them today.
 */
        ...
        <FlatList
          ...
          renderItem={({ item: rehearsal }) => (
            ...
            >
              ...
              <ThemedText type="small" testID={`rehearsal-status-${rehearsal.id}`}>
                {rehearsalStatusLabel(rehearsal.status)}
              </ThemedText>
              {rehearsal.start_date_time && (
                <ThemedText type="small">
                  {formatDayHeader(new Date(rehearsal.start_date_time))} {formatTime(rehearsal.start_date_time)}
                  {rehearsal.end_date_time ? `〜${formatTime(rehearsal.end_date_time)}` : ''}
                </ThemedText>
              )}
              {rehearsal.location && <ThemedText type="small">{rehearsal.location}</ThemedText>}
            </TouchableOpacity>
          )}
        />
      )}
